...
class HandEvaluator:
        ...
        def evaluate_percentile(hand):
            ...
            if len(hand) != 2:
                raise HandLengthException(
                    "Only 2-card hands are supported by the Two evaluator")
    ...
    class Five:
        ...
        def evaluate_rank(hand):
            ...
            if len(hand) != 5:
                raise HandLengthException(
                    "Only 5-card hands are supported by the Five evaluator")
    ...
    class Six:
        ...
        def evaluate_rank(hand):
            ...
            # except straight flush, so just need to determine the highest
            # flush
    ...
    class Seven:
        ...
        def evaluate_rank(hand):
            ...
            if len(hand) != 7:
                raise HandLengthException(
                    "Only 7-card hands are supported by the Seven evaluator")
            ...
            if flush_prime in LookupTables.Seven.prime_products_to_flush:
                flush_suit = LookupTables.Seven.prime_products_to_flush[
                    flush_prime]
    ...
    def evaluate_hand(hand, board=[]):
        ...
        if len(hand) not in hand_lengths:
            raise HandLengthException(
                "Only %s hole cards are supported" % ", ".join(map(str, hand_lengths)))
        ...
        if len(cards) == 2:
            ...
            raise HandLengthException(
                "Only 2, 5, 6, 7 cards total are supported by evaluate_hand")
        ...
        possible_opponent_hands = list(
            combinations(LookupTables.deck - set(cards), len(hand)))
        hands_beaten = 0
