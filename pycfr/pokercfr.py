...
class CounterfactualRegretMinimizer(object):
    ...
    def cfr(self):
        self.cfr_helper(self.tree.root, [{(): 1}
                        for _ in range(self.rules.players)])
    ...
    def cfr_holecard_node(self, root, reachprobs):
        ...
        next_reachprobs = [{hc: reachprobs[player][hc[0:prevlen]] / possible_deals for hc in root.children[0].holecards[player]}
                           for player in range(self.rules.players)]
        subpayoffs = self.cfr_helper(root.children[0], next_reachprobs)
        payoffs = [{hc: 0 for hc in root.holecards[player]}
                   for player in range(self.rules.players)]
    ...
    def cfr_boardcard_node(self, root, reachprobs):
        ...
        for bc in root.children:
            next_reachprobs = [{hc: reachprobs[player][hc] / possible_deals for hc in bc.holecards[player]}
                               for player in range(self.rules.players)]
    ...
    def cfr_action_node(self, root, reachprobs):
        ...
        action_probs = {hc: strategy.probs(self.rules.infoset_format(root.player, hc, root.board, root.bet_history))
                        for hc in reachprobs[root.player]}
        action_payoffs = [None, None, None]
        if root.fold_action:
            next_reachprobs[root.player] = {hc: action_probs[hc][FOLD] * reachprobs[root.player][hc]
                                            for hc in reachprobs[root.player]}
            action_payoffs[FOLD] = self.cfr_helper(
                root.fold_action, next_reachprobs)
        if root.call_action:
            next_reachprobs[root.player] = {hc: action_probs[hc][CALL] * reachprobs[root.player][hc]
                                            for hc in reachprobs[root.player]}
            action_payoffs[CALL] = self.cfr_helper(
                root.call_action, next_reachprobs)
        if root.raise_action:
            next_reachprobs[root.player] = {hc: action_probs[hc][RAISE] * reachprobs[root.player][hc]
                                            for hc in reachprobs[root.player]}
            action_payoffs[RAISE] = self.cfr_helper(
                root.raise_action, next_reachprobs)
        payoffs = []
        for player in range(self.rules.players):
            ...
            for i, subpayoff in enumerate(action_payoffs):
                ...
                for hc, winnings in subpayoff[player].iteritems():
                    # action_probs is baked into reachprobs for everyone except
                    # the acting player
    ...
    def cfr_strategy_update(self, root, reachprobs):
        if self.iterations == 0:
            ...
            for hc in root.holecards[root.player]:
                ...
                for i in range(3):
                    self.action_reachprobs[root.player][infoset][
                        i] += reachprobs[root.player][hc] * probs[i]
        ...
        for hc in root.holecards[root.player]:
            ...
            self.current_profile.strategies[
                root.player].policy[infoset] = probs
            for i in range(3):
                self.action_reachprobs[root.player][infoset][
                    i] += reachprobs[root.player][hc] * probs[i]
            self.profile.strategies[root.player].policy[infoset] = [self.action_reachprobs[root.player][
                infoset][i] / sum(self.action_reachprobs[root.player][infoset]) for i in range(3)]
    ...
    def cfr_regret_update(self, root, action_payoffs, ev):
        for i, subpayoff in enumerate(action_payoffs):
            ...
            for hc, winnings in subpayoff[root.player].iteritems():
                ...
                infoset = self.rules.infoset_format(
                    root.player, hc, root.board, root.bet_history)
                self.counterfactual_regret[root.player][
                    infoset][i] += immediate_cfr
...
class PublicChanceSamplingCFR(CounterfactualRegretMinimizer):
    ...
    def cfr_holecard_node(self, root, reachprobs):
        ...
        next_reachprobs = [{hc: reachprobs[player][hc[0:prevlen]] / possible_deals for hc in root.children[
            0].holecards[player] if not self.has_boardcard(hc)} for player in range(self.rules.players)]
        subpayoffs = self.cfr_helper(root.children[0], next_reachprobs)
        payoffs = [{hc: 0 for hc in reachprobs[player]}
                   for player in range(self.rules.players)]
        for player, subpayoff in enumerate(subpayoffs):
            for hand, winnings in subpayoff.items():
                hc = hand[0:prevlen]
    ...
    def cfr_strategy_update(self, root, reachprobs):
        ...
        for hc in reachprobs[root.player]:
            ...
            if sumpos_cfr == 0:
                ...
                # Use the strategy that's proportional to accumulated positive
                # CFR
                probs = [max(0, x) / sumpos_cfr for x in prev_cfr]
            # Use the updated strategy as our current strategy
            self.current_profile.strategies[
                root.player].policy[infoset] = probs
            # Update the weighted policy probabilities (used to recover the
            # average strategy)
            for i in range(3):
                self.action_reachprobs[root.player][infoset][
                    i] += reachprobs[root.player][hc] * probs[i]
            if sum(self.action_reachprobs[root.player][infoset]) == 0:
                ...
                self.profile.strategies[root.player].policy[
                    infoset] = self.equal_probs(root)
            else:
...
class ChanceSamplingCFR(CounterfactualRegretMinimizer):
    ...
    def cfr(self):
        ...
        todeal = random.sample(
            self.rules.deck, boardcards_per_hand + holecards_per_player * self.rules.players)
        # Deal holecards
        self.holecards = [tuple(todeal[p * holecards_per_player:(p + 1) * holecards_per_player])
    ...
    def cfr_action_node(self, root, reachprobs):
        ...
        if root.fold_action:
            next_reachprobs[root.player] = action_probs[
                FOLD] * reachprobs[root.player]
            action_payoffs[FOLD] = self.cfr_helper(
                root.fold_action, next_reachprobs)
        if root.call_action:
            next_reachprobs[root.player] = action_probs[
                CALL] * reachprobs[root.player]
            action_payoffs[CALL] = self.cfr_helper(
                root.call_action, next_reachprobs)
        if root.raise_action:
            next_reachprobs[root.player] = action_probs[
                RAISE] * reachprobs[root.player]
            action_payoffs[RAISE] = self.cfr_helper(
                root.raise_action, next_reachprobs)
        ...
        for i, subpayoff in enumerate(action_payoffs):
            ...
            for player, winnings in enumerate(subpayoff):
                # action_probs is baked into reachprobs for everyone except the
                # acting player
                if player == root.player:
    ...
    def cfr_strategy_update(self, root, reachprobs):
        ...
        # Update the weighted policy probabilities (used to recover the average
        # strategy)
        for i in range(3):
            self.action_reachprobs[root.player][infoset][
                i] += reachprobs[root.player] * probs[i]
        if sum(self.action_reachprobs[root.player][infoset]) == 0:
            # Default strategy is equal weight
            self.profile.strategies[root.player].policy[
                infoset] = self.equal_probs(root)
...
class OutcomeSamplingCFR(ChanceSamplingCFR):
    ...
    def cfr(self):
        ...
        # Call the standard CFR algorithm
        self.cfr_helper(
            self.tree.root, [1 for _ in range(self.rules.players)], 1.0)
    ...
    def cfr_action_node(self, root, reachprobs, sampleprobs):
        ...
        csp = self.exploration * (1.0 / len(root.children)) + (
            1.0 - self.exploration) * action_probs[action]
        payoffs = self.cfr_helper(
            root.get_child(action), reachprobs, sampleprobs * csp)
        # Update regret calculations
        self.cfr_regret_update(
            root, payoffs[root.player], action, action_probs[action])
    ...
    def cfr_strategy_update(self, root, reachprobs, sampleprobs):
        ...
        for i in range(3):
            self.action_reachprobs[root.player][infoset][
                i] += reachprobs[root.player] * probs[i] / sampleprobs
        if sum(self.action_reachprobs[root.player][infoset]) == 0:
            # Default strategy is equal weight
            self.profile.strategies[root.player].policy[
                infoset] = self.equal_probs(root)
