...
from pokertrees import (PublicTree, TerminalNode, CALL, FOLD, RAISE,
                        HolecardChanceNode, BoardcardChanceNode)
from _mathutils import choose
...
class StrategyProfile(object):
    ...
    def expected_value(self):
        ...
        if self.gametree.root is None:
            self.gametree.build()
        expected_values = self.ev_helper(
            self.gametree.root, [{(): 1} for _ in range(self.rules.players)])
    ...
    def old_ev_helper(self, root, pathprobs):
        ...
        if root.fold_action and probs[FOLD] > 0.0000000001:
            subpayoffs = self.ev_helper(
                root.fold_action, pathprob * probs[FOLD])
            for i, p in enumerate(subpayoffs):
                payoffs[i] += p
        if root.call_action and probs[CALL] > 0.0000000001:
            subpayoffs = self.ev_helper(
                root.call_action, pathprob * probs[CALL])
            for i, p in enumerate(subpayoffs):
                payoffs[i] += p
        if root.raise_action and probs[RAISE] > 0.0000000001:
            subpayoffs = self.ev_helper(
                root.raise_action, pathprob * probs[RAISE])
            for i, p in enumerate(subpayoffs):
                payoffs[i] += p
        return payoffs
    ...
    def ev_action_node(self, root, reachprobs):
        ...
        if root.raise_action:
            next_reachprobs[root.player] = {hc: action_probs[hc][RAISE] * reachprobs[root.player][hc]
                                            for hc in root.holecards[root.player]}
            action_payoffs[RAISE] = self.ev_helper(
            ...
            for action, subpayoff in enumerate(action_payoffs):
                ...
                if root.player == player:
                    for hc, winnings in subpayoff[player].iteritems():
                        player_payoffs[
                            hc] += winnings * action_probs[hc][action]
                else:
                    for hc, winnings in subpayoff[player].iteritems():
    ...
    def best_response(self):
        ...
        expected_values = self.br_helper(self.publictree.root, [
                                         {(): 1} for _ in range(self.rules.players)], responses)
        for ev in expected_values:
            assert(len(ev) == 1)
        expected_values = tuple(next(ev.itervalues())
                                for ev in expected_values)  # pull the EV from the dict returned
    ...
    def br_holecard_node(self, root, reachprobs, responses):
        ...
        next_reachprobs = [{hc: reachprobs[player][hc[0:prevlen]] / possible_deals for hc in root.children[0].holecards[player]}
                           for player in range(self.rules.players)]
        subpayoffs = self.br_helper(
            root.children[0], next_reachprobs, responses)
        payoffs = [{hc: 0 for hc in root.holecards[player]}
                   for player in range(self.rules.players)]
        for player, subpayoff in enumerate(subpayoffs):
            for hand, winnings in subpayoff.items():
    ...
    def br_action_node(self, root, reachprobs, responses):
        ...
        if root.fold_action:
            next_reachprobs[root.player] = {hc: action_probs[hc][FOLD] * reachprobs[root.player][hc]
                                            for hc in root.holecards[root.player]}
            action_payoffs[FOLD] = self.br_helper(
                root.fold_action, next_reachprobs, responses)
        if root.call_action:
            next_reachprobs[root.player] = {hc: action_probs[hc][CALL] * reachprobs[root.player][hc]
                                            for hc in root.holecards[root.player]}
            action_payoffs[CALL] = self.br_helper(
                root.call_action, next_reachprobs, responses)
        if root.raise_action:
            next_reachprobs[root.player] = {hc: action_probs[hc][RAISE] * reachprobs[root.player][hc]
                                            for hc in root.holecards[root.player]}
            action_payoffs[RAISE] = self.br_helper(
                root.raise_action, next_reachprobs, responses)
        payoffs = []
        for player in range(self.rules.players):
            if player is root.player:
                payoffs.append(
                    self.br_response_action(root, responses, action_payoffs))
            else:
                player_payoffs = {hc: 0 for hc in root.holecards[player]}
