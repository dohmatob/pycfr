...
from ..pokergames import (leduc_rules, leduc_eval, leduc_format,
                          half_street_kuhn_rules, half_street_kuhn_gametree)
...
def test_noaction_game():
    ...
    no_action_rules = GameRules(players, deck, rounds, ante,
                                blinds, handeval=leduc_eval, infoset_format=leduc_format)
    ...
    # print 'Expected values: [{0:.9f},{1:.9f}]'.format(expected_value[0],
    # expected_value[1])
    print 'Expected values: [{0},{1}]'.format(expected_value[0], expected_value[1])
    assert(expected_value >= (-0.0001, -0.0001)
           and expected_value <= (0.0001, 0.0001))
...
def test_half_street_kuhn():
    hskuhn_rules = half_street_kuhn_rules()
    hskuhn_gametree = half_street_kuhn_gametree()
    
    s0 = Strategy(0)
    s0.policy = {'A:/:': [0, 0, 1], 'K:/:': [
        0, 1, 0], 'Q:/:': [0, 2.0 / 3.0, 1.0 / 3.0]}
    
    s1 = Strategy(1)
    s1.policy = {'A:/r:': [0, 1, 0], 'K:/r:': [2.0 / 3.0, 1.0 / 3.0, 0], 'Q:/r:': [
        1, 0, 0], 'A:/c:': [0, 1, 0], 'K:/c:': [0, 1, 0], 'Q:/c:': [0, 1, 0]}
    
    eq0 = Strategy(0)
    eq0.build_default(hskuhn_gametree)
    # eq0.save_to_file('hskuhn_eq0.strat')
    
    eq1 = Strategy(1)
    eq1.build_default(hskuhn_gametree)
    # eq1.save_to_file('hskuhn_eq1.strat')
    ...
    overbet0 = Strategy(0)
    overbet0.policy = {
        'Q:/:': [0, 0.5, 0.5], 'K:/:': [0, 1, 0], 'A:/:': [0, 0, 1]}
    overbet1 = Strategy(1)
    overbet1.policy = {'Q:/c:': [0, 1, 0], 'K:/c:': [0, 1, 0], 'A:/c:': [
        0, 1, 0], 'Q:/r:': [1, 0, 0], 'K:/r:': [0.5, 0.5, 0], 'A:/r:': [0, 1, 0]}
...
def test_leduc():
    ...
    # br.save_to_file('leduc_nash0_br.strat')
    ...
    # br.save_to_file('leduc_nash1_br.strat')
    ...
    profile = StrategyProfile(rules, [rand0, rand1])
    result = profile.best_response()
    br = result[0].strategies[0]
    ev = result[1][0]
